...
namespace yockcraft {
  ...
  Shader::Shader(const std::string& vertex_path , const std::string& fragment_path ,
      ...
      : vertex_path(vertex_path) , fragment_path(fragment_path) , geometry_path(geom_path) {
    vertex_src = utils::ReadFile(vertex_path);
    fragment_src = utils::ReadFile(fragment_path);

    if (geometry_path.has_value()) {
      geometry_src = utils::ReadFile(geometry_path.value());
    } else {
      geometry_src = "";
    }
    ...
    if (geometry_path.has_value()) {
      gsrc_ptr = geometry_src.c_str();
    }
  
    uint32_t vertex_shader = glCreateShader(GL_VERTEX_SHADER);
    uint32_t fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
    uint32_t geom_shader = 0;
    ...
    if (geometry_path.has_value()) {
      glShaderSource(geom_shader , 1 , &gsrc_ptr , nullptr);
    }
    ...
    if (geometry_path.has_value()) {
      if (!CompileShader(geom_shader , gsrc_ptr)) {
        glDeleteShader(vertex_shader);
        if (geometry_path.has_value()) {
          glDeleteShader(geom_shader);
        }
    ...
    if (geometry_path.has_value()) {
      glAttachShader(renderer_id , geom_shader);
    }
